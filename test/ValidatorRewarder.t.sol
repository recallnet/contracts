...
abstract contract ValidatorRewarderTestBase is Test {
    ...
    function setUp() public virtual {
        ...
        rewarder = rewarderDeployer.runWithParams(
            "local",
            address(token),
            subnet,
            CHECKPOINT_PERIOD
    ...
    ) internal pure returns (Consensus.ValidatorData memory) {
        return Consensus.ValidatorData({
            validator: validator,
            blocksCommitted: uint64(blocksCommitted)
        });
    }
...
contract ValidatorRewarderSubnetTest is ValidatorRewarderTestBase {
    ...
    function testInitializeWithInvalidPeriod() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(ValidatorRewarder.InvalidCheckpointPeriod.selector, 0)
        );                
...
contract ValidatorRewarderBasicClaimTest is ValidatorRewarderTestBase {
    ...
    function testNotifyValidClaimInvalidSubnet() public {
        SubnetID memory wrongSubnet = SubnetID(ROOTNET_CHAINID, new address[](0))
            .createSubnetId(address(0x123));
...
contract ValidatorRewarderComplexClaimTest is ValidatorRewarderTestBase {
    ...
    function testNotifyValidClaimConcurrentClaims() public {
        ...
        assertApproxEqAbs(
            token.balanceOf(claimants[1]),
            (154712667492044 * 2) + (154712811108101 * 2),
            1000
        );
        vm.stopPrank();
        ...
        assertApproxEqAbs(
            token.balanceOf(claimants[2]),
            (154712667492044 * 3) + (154712811108101 * 3),
            1000
        );
        vm.stopPrank();
...
contract ValidatorRewarderFFITest is ValidatorRewarderTestBase {
    ...
    function testRewardCalculationWithFFI() public {
        ...
        for (uint i = 0; i < numCheckpoints; i++) {
            ...
            for (uint j = 0; j < claimants.length; j++) {
                ...
                if (j == 0) {
                    ...
                    assertApproxEqAbs(
                        ...
                        1000,
                        string.concat("Rewarder balance mismatch on first claim at checkpoint ", vm.toString(currentCheckpoint))
                    );       
                    ...
                    assertApproxEqAbs(
                        ...
                        expectedValidatorShare,
                        1000,
                        string.concat("Rewarder balance mismatch on subsequent claim at checkpoint ", vm.toString(currentCheckpoint))
                    ); 
                }
            }
