...
library CBORDataStructures {
    ...
    function expandArray(
        ...
        // Count up how many keys we have, set cursor
        (uint totalItems, uint dataStart, ) = getDataStructureItemLength(
            encoding,
            arrayCursor,
            Spec.MajorType.Array,
            shortCount
        );
        ...
        returns (uint256 totalItems, uint256 dataStart, uint256 dataEnd)
    {
        ...
        if (shortCount == 31) {
            ...
            (totalItems, dataEnd) = Utils.scanIndefiniteItems(
                encoding,
                countEnd,
                0
            );
            ...
            if (majorType == Spec.MajorType.Map) totalItems *= 2;
            // Data starts right where count ends (which is cursor+1)
            dataStart = countEnd;
